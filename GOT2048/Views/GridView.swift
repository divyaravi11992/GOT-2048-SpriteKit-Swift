...
class GridView: SKSpriteNode {
    ...
    func gridPosition(row:Int, col:Int) -> CGPoint {
        ...
        let colSize = CGFloat(col) * blockSize - (blockSize * CGFloat(cols)) / 2
        let rowSize = CGFloat(rows - row - 1) * blockSize - (blockSize * CGFloat(rows)) / 2.0
        let x = colSize + xOffset
        let y = rowSize + yOffset
